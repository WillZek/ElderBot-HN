...
global.authFile = 'ElderSession'
global.authFileJB  = 'ElderJadiBot'
...
mobile: MethodMobile, 
browser: opcion == '1' ? ['ElderBot-ST', 'Edge', '20.0.04'] : methodCodeQR ? ['ElderBot-ST', 'Edge', '20.0.04'] : ["Ubuntu", "Chrome", "20.0.04"],
auth: {
creds: state.creds,
...
if (opts['autocleartmp'] && (global.support || {}).find) (tmp = [os.tmpdir(), 'tmp', "ElderJadiBot"], tmp.forEach(filename => cp.spawn('find', [filename, '-amin', '2', '-type', 'f', '-delete'])))}, 30 * 1000)}
...
const subBotDirectory = './ElderJadiBot';
if (!existsSync(subBotDirectory)) {
console.log('🍭 ElderBot-HN no tiene Sub-Bots vinculados.');
return;
}
...
const mainBotAuthFile = 'ElderSession';
try {
const mainBot = await connectionUpdate(mainBotAuthFile);
global.conns.push(mainBot);
console.log(chalk.bold.greenBright(`🍭 ElderBot conectado correctamente.`))

await connectSubBots();
} catch (error) {
console.error(chalk.bold.cyanBright(`🥀 Error al iniciar ElderBot-ST: `, error))
}
})();
...
let prekey = []
let directorio = readdirSync("./ElderSession")
let filesFolderPreKeys = directorio.filter(file => {
return file.startsWith('pre-key-')
})
prekey = [...prekey, ...filesFolderPreKeys]
filesFolderPreKeys.forEach(files => {
unlinkSync(`./ElderSession/${files}`)
})
} 
...
const directories = ['./ElderSession/', './ElderJadiBot/']
